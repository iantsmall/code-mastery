...
*/
 ...
 export default function jump(nums: number[]): number {
     if(nums.length === 1){
        return 0; // immediately return, we've started at the end
     }
     const recursiveJump = (idx: number): number => {
        ...
        const nM = idx + nums[idx];
        for(let i = idx; i < nM ; i++){

        }
        return recursiveJump(furthestReachingIdx);
     };
